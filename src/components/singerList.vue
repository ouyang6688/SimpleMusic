<template>
  <div>
    ...
    <div class="topheader">
      ...
      <div class="title">
        {{ playlist.name }} ({{ playlist.alias[0] }})
      </div>
    ...
    <dl class="musiclist">
      <dt>
        播放全部 <span>(共{{ singers.length }}首)</span>
      </dt>
      <dd v-for="(item,index) in singers" :key="index" @click="addlists(index)">
        <h1>{{ index + 1 }}</h1>
        <div class="info">
          <h2>{{ item.name }}</h2>
          <h3>
    ...
    <div class="newcomment" v-show="hotcomment.length">
      <dl>
        <dt>最新评论</dt>
...
export default {
  ...
  methods: {
    getDetailFun() {
      ...
      // console.log(id);
      getArtists({id: id}).then(data => {
        console.log(data);
        this.playlist = data.artist;
        this.singers = data.hotSongs;
      })
      getMusicLists({id: id}).then(data => {
        console.log(data)
